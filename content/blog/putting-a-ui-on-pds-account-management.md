...
Abramov's recent post [*A Social Filesystem*](https://overreacted.io/a-social-filesystem/)
...
## The problem: it all lives in a terminal
...
itself runs **client-side**. You — or your browser tab — drive a stateful, hard-to-undo
...
Migration is the sharp edge. The flow exports your repo, creates an account on the new
server, imports everything (blobs included — MOOver uploads them one at a time, in the tab,
and it takes a while), rewrites your DID, and activates. If any of that stalls — your laptop
sleeps, the network drops, a step errors halfway — there's **no resume, no pause, no
checkpoint**. You're left in a partial state, and in the worst case looking at what feels
like data loss. We hit exactly this ourselves (more below).
...
a one-shot command on your laptop.
...
Listing accounts is `com.atproto.sync.listRepos` enriched with
`com.atproto.admin.getAccountInfos`. Password reset is
`com.atproto.server.requestPasswordReset`. Delete is `com.atproto.admin.deleteAccount`. The
PDS admin authenticates with plain HTTP Basic auth, so the one rule that matters is: keep
...
We haven't built these yet, and we're not claiming to be the only ones who could. The
surface is there; we'd rather build it as a UI than keep typing it into a terminal.
...
to miss. Until you flip it, the account is fully migrated *and completely invisible*: the
AppView returns `AccountDeactivated`, the profile 404s, and it looks like the data is gone.
It isn't — but nothing tells you that.
...
they go sideways. A stateful flow that knows it's mid-migration — that can show you "you're
activated but not yet visible, here's why" — is exactly what we want to build.

## Where this fits

Account management is milestone one on the way from managed hosting to a real PDS management
interface — and eventually that personal-social-cloud corner of the atmosphere. We're
keeping it open: the code lands on Tangled under
[@eny.social](https://bsky.app/profile/eny.social), because the problems here aren't
specific to us.
